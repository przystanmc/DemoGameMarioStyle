...
# Szkielet świeci słabiej i bardziej "zimno" niż demon
@export var max_light_energy: float = 0.5

@export var target: Node2D
# Szkielet ma mniejszy zasięg wykrywania niż demon
@export var detect_range := 50.0
@export var attack_range := 15.0

# --- PARAMETRY RUCHU ---
# Szkielet jest wolniejszy niż demon
var speed_walk := 25.0
var speed_run := 55.0
var health := 10
...
func _physics_process(delta):
	...
	if not is_hit and not is_attacking:
		...
		if dist < detect_range:
			...
			# patrol_timer odlicza tylko poza CHASE
...
func _update_light(delta):
	if not light_node:
		return
	# Szkielet pulsuje wolno i nierówno — efekt "martwego" światła
	light_pulse_time += delta * 3.0
	var flicker = sin(light_pulse_time) * 0.06 * night_level
	target_energy = lerp(target_energy, night_level * max_light_energy, 0.04)
...
func perform_ai_attack():
	...
	await get_tree().create_timer(0.8).timeout  # Szkielet atakuje wolniej niż demon
	if not is_dead:
		can_attack = true
...
func take_damage(_attacker_pos = Vector2.ZERO):
	...
	velocity.x = knockback_dir * 150  # Szkielet lżejszy — większy odrzut
	velocity.y = -80
...
func die():
	...
	if light_node:
		...
		light_tween.tween_property(light_node, "energy", 0.0, 0.3)

...
func _update_light(delta):
	if not light_node:
		return
	light_pulse_time += delta * 7.0
	var flicker = sin(light_pulse_time) * 0.07 * night_level
...
func perform_attack():
	...
	# Poprawiony zamach — back_x różni się od target_x
...
func perform_ai_attack():
	can_attack = false
	# await — cooldown zaczyna się po zakończeniu animacji
